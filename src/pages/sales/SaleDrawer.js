...
import { closeAddSaleForm, SaleTabView, SaleServiceApi, SaleServiceSearchName, SaleProductApi, SaleProductSearchName, SaleVoucherApi, SaleMembershipApi, CloseCheckoutForm, SaleSubscriptionApi } from "store/slices/saleSlice";
...
import SaleSubscriptionGridView from "./List/SaleSubscriptionGridView";
...
const SaleDrawer = (props) => {
  ...
  const isSubscription = useSelector((state) => state.sale.isSubscription);
  ...
  const fetchDataSaleSubscription = () => {
    dispatch(SaleSubscriptionApi({ next_page_url: isSubscription.next_page_url }));
  };
  const [isFetchingSubscription, setIsFetchingSubscription] = useState(false);
  const loadMoreSubscription = () => {
