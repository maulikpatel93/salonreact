...
import { CloseAddCloseddateForm, CloseddateUpdateApi, reset } from "store/slices/closedateSlice";
import moment from "moment";

const CloseDateEditForm = () => {
  ...
  const rightDrawerOpened = useSelector((state) => state.closedate.isOpenedEditForm);
  const detail = useSelector((state) => state.closedate.isDetailData);
  ...
  const handleClosedDateSubmit = (values, { setErrors, setStatus, setSubmitting, resetForm }) => {
    setLoading(true);
    try {
      dispatch(CloseddateUpdateApi(values)).then((action) => {
      ...
      <React.Fragment>
        <Formik enableReinitialize={false} initialValues={initialValues} validationSchema={validationSchema} onSubmit={handleClosedDateSubmit}>
          {(formik) => {
            useEffect(() => {
              if (detail) {
                const fields = ["id", "start_date", "end_date", "reason"];
...
export default CloseDateEditForm;
