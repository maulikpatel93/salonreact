...
const create = (values) => {
  ...
  const action = "afterlogin/membership/store";
...
const update = (values) => {
  ...
  const action = "afterlogin/membership/update/" + values.id;
  formData.append("auth_key", auth_key);
  formData.append("action", action);
  formData.append("salon_id", auth.user.salon_id);
  return axios.post(API_URL + action, formData, { headers: authHeader({ contentType: "multipart/form-data" }) });
...
const view = (values) => {
  ...
  const action = page ? `afterlogin/membership/view?page=${page}&${sortstring}` : `afterlogin/membership/view?${sortstring}`;
  const data = {
    auth_key: auth_key,
    action: action,
...
const deleted = (values) => {
  const auth = store.getState().auth;
  const auth_key = auth.user.auth_key;
  const action = `afterlogin/membership/delete/${values.id}`;
  const data = {
    auth_key: auth_key,
    action: action,
  };
...
const suggetionlist = (values) => {
  ...
  const action = page ? `afterlogin/membership/view?page=${page}&q=${q}` : `afterlogin/membership/view?q=${q}`;
  const data = {
    auth_key: auth_key,
    action: action,
    salon_id: auth.user.salon_id,
...
export default membershipApiController;
