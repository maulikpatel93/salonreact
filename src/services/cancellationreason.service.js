...
const create = (values) => {
  ...
  const action = "afterlogin/cancellationreason/store";
...
const update = (values) => {
  ...
  const action = "afterlogin/cancellationreason/update/" + values.id;
  formData.append("auth_key", auth_key);
  formData.append("action", action);
...
const view = (values) => {
  ...
  const action = page ? `afterlogin/cancellationreason/view?page=${page}&${sortstring}` : `afterlogin/cancellationreason/view?${sortstring}`;
  const data = {
    auth_key: auth_key,
    action: action,
...
const deleted = (values) => {
  const auth = store.getState().auth;
  const auth_key = auth.user.auth_key;
  const action = `afterlogin/cancellationreason/delete/${values.id}`;
  const data = {
    auth_key: auth_key,
    action: action,
  };
...
const cancellationreasonApiController = {
  create,
  update,
  view,
  deleted,
};
export default cancellationreasonApiController;
