...
const update = (values) => {
  ...
  const action = "afterlogin/subscription/update/" + values.id;
  formData.append("auth_key", auth_key);
  formData.append("action", action);
...
const deleted = (values) => {
  const auth = store.getState().auth;
  const auth_key = auth.user.auth_key;
  const action = `afterlogin/subscription/delete/${values.id}`;
  const data = {
    auth_key: auth_key,
    action: action,
  };
...
const suggetionlist = (values) => {
  ...
  const action = page ? `afterlogin/subscription/view?page=${page}&q=${q}` : `afterlogin/subscription/view?q=${q}`;
  const data = {
    auth_key: auth_key,
    action: action,
    salon_id: auth.user.salon_id,
