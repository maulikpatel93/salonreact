...
const create = (values) => {
  ...
  const action = "afterlogin/voucher/store";
...
const update = (values) => {
  ...
  const action = "afterlogin/voucher/update/" + values.id;
  formData.append("auth_key", auth_key);
  formData.append("action", action);
...
const view = (values) => {
  ...
  const action = page ? `afterlogin/voucher/view?page=${page}&${sortstring}` : `afterlogin/voucher/view?${sortstring}`;
  const data = {
    auth_key: auth_key,
    action: action,
    ...
    field: values && values.id ? "" : "code,name,description,amount,valid,used_online,limit_uses,limit_uses_value,is_active", // first_name,last_name,email
...
const deleted = (values) => {
  const auth = store.getState().auth;
  const auth_key = auth.user.auth_key;
  const action = `afterlogin/voucher/delete/${values.id}`;
  const data = {
    auth_key: auth_key,
    action: action,
  };
...
const suggetionlist = (values) => {
  ...
  const action = page ? `afterlogin/voucher/view?page=${page}&q=${q}` : `afterlogin/voucher/view?q=${q}`;
  const data = {
    auth_key: auth_key,
    action: action,
    salon_id: auth.user.salon_id,
...
export default voucherApiController;
