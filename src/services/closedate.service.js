...
const create = (values) => {
  ...
  const action = "afterlogin/closedate/store";
...
const update = (values) => {
  ...
  const action = "afterlogin/closedate/update/" + values.id;
  formData.append("auth_key", auth_key);
  formData.append("action", action);
...
const view = (values) => {
  ...
  const action = page ? `afterlogin/closedate/view?page=${page}&${sortstring}` : `afterlogin/closedate/view?${sortstring}`;
  const data = {
    auth_key: auth_key,
    action: action,
    ...
    field: values && values.id ? "" : "start_date,end_date,reason", // first_name,last_name,email
...
const deleted = (values) => {
  const auth = store.getState().auth;
  const auth_key = auth.user.auth_key;
  const action = `afterlogin/closedate/delete/${values.id}`;
  const data = {
    auth_key: auth_key,
    action: action,
  };
...
const closedateApiController = {
  create,
  update,
  view,
  deleted,
};
export default closedateApiController;
